// ============================================================
// 合同会社XEN — ミッションセクション Neural Network 背景演出
// アップロードされたデモ(フルスクリーン・操作型)を、
// QUOTE/MISSIONセクションの背景に収まる形にアダプトしたもの。
//
// 主な変更点（元コードから）:
//  - フルページ(window基準)ではなく、#mission-neural セクションの
//    サイズを基準にレンダラー/カメラ/クリック座標を計算
//  - ページスクロールを邪魔しないよう controls.enableZoom = false
//    （ホイール操作はページスクロールに譲る）
//  - 画面外にスクロールしている間は描画ループを止めて負荷を軽減
//  - prefers-reduced-motion 環境では自動回転・アイドルアニメーションを止める
//    （クリックによるパルス演出はユーザー操作起点なので維持）
//  - UIパネルはセクション内に小さく配置し、サイト本体のフォント変数を使用
//  - フォーメーション生成ロジック自体は元コードを尊重しほぼそのまま
// ============================================================
...
const container = document.getElementById('mission-neural');
const canvasElement = document.getElementById('quote-neural-canvas');
if (!container || !canvasElement) {
  // このセクションが存在しないページでは何もしない
} else {
  ...
  // ブランドカラーを基調にした4テーマ（XENのteal/indigo/terraを1番目に採用）
  const colorPalettes = [
    [new THREE.Color(0x14B8A6), new THREE.Color(0x6B5FFF), new THREE.Color(0xFF6B4A), new THREE.Color(0x5347E0), new THREE.Color(0x2DD4BF)],
  ...
  function cameraDistanceFor(aspect) {
    // セクションは横長・縦が短いため、縦長(フルスクリーン)前提のカメラ距離だと
    // 上下が大きく見切れる。アスペクト比に応じてカメラを遠ざけて全体を収める。
    if (aspect > 2.2) return 40;
    if (aspect > 1.6) return 32;
    if (aspect > 1.1) return 26;
    return 20;
  }
  ...
  const camera = new THREE.PerspectiveCamera(60, initW / initH, 0.1, 1200);
  camera.position.set(0, 4, cameraDistanceFor(initW / initH));
  ...
  renderer.setClearColor(0x000000, 0); // 透過にしてセクションの黒背景をそのまま活かす
  renderer.outputColorSpace = THREE.SRGBColorSpace;

  function createStarfield() {
    const count = 2200, pos = [];
    ...
    const mat = new THREE.PointsMaterial({ color: 0xffffff, size: 0.15, sizeAttenuation: true, depthWrite: false, opacity: 0.7, transparent: true });
  ...
  controls.enableZoom = false; // ページのスクロールを横取りしないようホイールズームは無効化

  const composer = new EffectComposer(renderer);
  composer.addPass(new RenderPass(scene, camera));
  const bloomPass = new UnrealBloomPass(new THREE.Vector2(initW, initH), 1.3, 0.4, 0.7);
  composer.addPass(bloomPass);
  const filmPass = new FilmPass(0.3, 0.5, 2048, false);
  ...
  function generateNeuralNetwork(formationIndex, densityFactor = 1.0) {
    ...
    function generateQuantumCortex() {
      ...
      const primaryAxes = 6, nodesPerAxis = 8, axisLength = 18;
      const axisEndpoints = [];
      ...
      const ringDistances = [4.5, 9, 13.5];
      const ringNodes = [];
      ...
      for (let r = 0; r < ringNodes.length - 1; r++) {
        ...
        for (let i = 0; i < connectionsCount; i++) {
          ...
          if (!innerNode.isConnectedTo(outerNode)) innerNode.addConnection(outerNode, 0.6);
        }
      }
      for (let i = 0; i < axisEndpoints.length; i++) {
        ...
        for (let j = 1; j <= numIntermediates; j++) {
          ...
          pos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(3), THREE.MathUtils.randFloatSpread(3), THREE.MathUtils.randFloatSpread(3)));
    ...
    function generateHyperdimensionalMesh() {
      rootNode = new Node(new THREE.Vector3(0, 0, 0), 0, 0); rootNode.size = 1.5; nodes.push(rootNode);
      const dimensions = 4;
      const nodesPerDimension = Math.floor(34 * densityFactor);
      const maxRadius = 18;
      ...
      for (let d = 0; d < dimensions; d++) {
        ...
        for (let i = 0; i < nodesPerDimension; i++) {
          ...
          const biasedVec = new THREE.Vector3().addVectors(dimVec.clone().multiplyScalar(0.6 + Math.random() * 0.4), randomVec.clone().multiplyScalar(0.3)).normalize();
        ...
        for (let layer = 0; layer < layers; layer++) {
          ...
          for (let i = startIdx; i < endIdx; i++) {
            ...
            for (let j = 0; j < Math.min(connectionsCount, nearbyNodes.length); j++) {
              if (!node.isConnectedTo(nearbyNodes[j])) node.addConnection(nearbyNodes[j], 0.4 + Math.random() * 0.4);
            }
            if (layer > 0) {
              const prevLayer = dimNodes.slice((layer - 1) * nodesPerLayer, layer * nodesPerLayer)
                .sort((a, b) => node.position.distanceTo(a.position) - node.position.distanceTo(b.position));
              if (prevLayer.length > 0 && !node.isConnectedTo(prevLayer[0])) node.addConnection(prevLayer[0], 0.8);
            }
          }
        }
      }
        ...
        for (let d2 = d1 + 1; d2 < dimensions; d2++) {
          ...
          for (let i = 0; i < connectionsCount; i++) {
            ...
            if (!n1.isConnectedTo(n2)) {
              ...
              midPos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(2), THREE.MathUtils.randFloatSpread(2), THREE.MathUtils.randFloatSpread(2)));
      ...
      const jumpConnections = Math.floor(8 * densityFactor);
      for (let i = 0; i < jumpConnections; i++) {
        ...
        if (!startNode.isConnectedTo(endNode)) {
          ...
          for (let j = 1; j < numPoints; j++) {
            const t = j / numPoints;
            const pos = new THREE.Vector3().lerpVectors(startNode.position, endNode.position, t);
            pos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(8) * Math.sin(t * Math.PI), THREE.MathUtils.randFloatSpread(8) * Math.sin(t * Math.PI), THREE.MathUtils.randFloatSpread(8) * Math.sin(t * Math.PI)));
    ...
    function generateNeuralVortex() {
      ...
      const numSpirals = 6, totalHeight = 26, maxRadius = 14;
      const nodesPerSpiral = Math.floor(26 * densityFactor);
      const spiralNodes = [];
      for (let s = 0; s < numSpirals; s++) {
        ...
        for (let i = 0; i < nodesPerSpiral; i++) {
          ...
          pos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(1.5), THREE.MathUtils.randFloatSpread(1.5), THREE.MathUtils.randFloatSpread(1.5)));
      ...
      for (const spiral of spiralNodes) {
        ...
        for (let i = 0; i < spiral.length - 1; i++) spiral[i].addConnection(spiral[i + 1], 0.9);
      }
      for (let s = 0; s < numSpirals; s++) {
      ...
      for (let r = 0; r < ringLevels; r++) {
        ...
        const ringNodesAtHeight = nodes.filter((n) => n !== rootNode && Math.abs(n.position.y - height) < 2);
        ringNodesAtHeight.sort((a, b) => Math.atan2(a.position.z, a.position.x) - Math.atan2(b.position.z, b.position.x));
        if (ringNodesAtHeight.length > 3) {
          for (let i = 0; i < ringNodesAtHeight.length; i++) ringNodesAtHeight[i].addConnection(ringNodesAtHeight[(i + 1) % ringNodesAtHeight.length], 0.5);
        }
      }
      const radialConnections = Math.floor(8 * densityFactor);
      const candidates = nodes.filter((n) => n !== rootNode && n.position.length() > 5).sort(() => Math.random() - 0.5).slice(0, radialConnections);
      for (const node of candidates) {
        ...
        for (let i = 1; i <= numSegments; i++) {
          ...
          segPos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(2), THREE.MathUtils.randFloatSpread(2), THREE.MathUtils.randFloatSpread(2)));
    ...
    function generateSynapticCloud() {
      rootNode = new Node(new THREE.Vector3(0, 0, 0), 0, 0); rootNode.size = 1.5; nodes.push(rootNode);
      const numClusters = 6, maxDist = 16;
      const clusterNodes = [];
      ...
      for (const cluster of clusterNodes) {
        const clusterSize = Math.floor(16 * densityFactor);
        const cloudRadius = 6 + Math.random() * 3;
        for (let i = 0; i < clusterSize; i++) {
          ...
          const nearbyNodes = nodes.filter((n) => n !== newNode && n !== cluster && n.clusterRef === cluster && n.position.distanceTo(pos) < cloudRadius * 0.4);
      ...
      const interClusterCount = Math.floor(12 * densityFactor);
      for (let i = 0; i < interClusterCount; i++) {
        ...
        bridgePos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(5), THREE.MathUtils.randFloatSpread(5), THREE.MathUtils.randFloatSpread(5)));
      ...
      const longRangeCount = Math.floor(8 * densityFactor);
      const outerNodes = nodes.filter((n) => n.distanceFromRoot > maxDist * 0.6).sort(() => Math.random() - 0.5).slice(0, longRangeCount);
      for (const outerNode of outerNodes) {
        ...
        for (let i = 1; i <= numSegments; i++) {
          ...
          segPos.add(new THREE.Vector3(THREE.MathUtils.randFloatSpread(4), THREE.MathUtils.randFloatSpread(4), THREE.MathUtils.randFloatSpread(4)));
  ...
  renderer.domElement.addEventListener('touchstart', (e) => {
    if (e.touches.length > 0 && !config.paused) {
      // 1本指タップはパルス、ドラッグはOrbitControls側で回転として処理される
      triggerPulse(e.touches[0].clientX, e.touches[0].clientY);
    }
  }, { passive: true });

  // ── UIパネル（テーマ・密度・操作ボタン） ──
  const themeButtons = container.querySelectorAll('.qnn-theme-btn');
  ...
  const densitySlider = document.getElementById('qnn-density-slider');
  const densityValue = document.getElementById('qnn-density-value');
  let densityTimeout;
  if (densitySlider) {
    densitySlider.addEventListener('input', (e) => {
      ...
      densityTimeout = setTimeout(() => createNetworkVisualization(config.currentFormation, config.densityFactor), 300);
    });
  }

  const changeFormationBtn = document.getElementById('qnn-formation-btn');
  const pausePlayBtn = document.getElementById('qnn-pause-btn');
  const resetCameraBtn = document.getElementById('qnn-reset-btn');
  ...
  if (changeFormationBtn) {
    changeFormationBtn.addEventListener('click', (e) => {
      ...
      setTimeout(() => { controls.autoRotate = !prefersReducedMotion; }, 2000);
    });
  }
  if (pausePlayBtn) {
    pausePlayBtn.addEventListener('click', (e) => {
      ...
      pausePlayBtn.textContent = config.paused ? '再生' : '一時停止';
      controls.autoRotate = config.paused ? false : !prefersReducedMotion;
    });
  }
  if (resetCameraBtn) {
    resetCameraBtn.addEventListener('click', (e) => {
      ...
      controls.reset();
      controls.autoRotate = false;
      setTimeout(() => { controls.autoRotate = !prefersReducedMotion; }, 1500);
    });
  }
  ...
  // 画面外にスクロールしている間は重い描画(ブルーム/フィルムパス含む)を止め、負荷を抑える
  ...
  function animate() {
    ...
    if (!config.paused) {
      ...
      if (!prefersReducedMotion) starField.rotation.y += 0.0003;
    }
    controls.update();
    composer.render();
  }
  ...
  function onResize() {
    ...
    if (w === 0 || h === 0) return;
    camera.aspect = w / h;
    camera.position.z = cameraDistanceFor(w / h) * Math.sign(camera.position.z || 1) || cameraDistanceFor(w / h);
